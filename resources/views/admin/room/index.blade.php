...
@section('content')
    @include('admin.layouts.components.header', [
        'title' => __('messages.list', [
            'name' => trans_choice('content.room', 2),
        ]),
        'breadcrumbs' => Breadcrumbs::render('admin.rooms.index'),
        'filter' => false,
        'create_btn' => [
            'status' => true,
            'route' => route('admin.rooms.create'),
...
@push('scripts')
    <script>
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row, meta) {
                            ...
                            var edit_url = `{{ url('/') }}/admin/rooms/` + row['id'] +
                                `/edit/?tab=edit`;
                            var show_url = `{{ url('/') }}/admin/rooms/` + row['id'] +
                                `?tab=details`;
        ...
        $(document).on('click', '.clsstatus', function() {
            ...
            var url = `{{ url('/') }}/admin/rooms/status/` + id + `/` + status;
            tableChnageStatus(url, oTable);
        });
        $(document).on('click', '.clsdelete', function() {
            var id = $(this).attr('data-id');
            var e = $(this).parent().parent();
            var url = `{{ url('/') }}/admin/rooms/` + id;
